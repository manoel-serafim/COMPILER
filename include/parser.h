...
typedef enum {IF_SK, WHILE_SK, RETURN_SK, ASSIGN_SK/*=2*/, VAR_SK, VECT_SK, FUNCT_SK, CALL_SK, PARAM_SK, VECT_PARAM_SK} stmt_kind;
typedef enum {OP_EK, ID_EK, NUM_EK, TYPE_EK/*ie. int decl*/, VECT_ID_EK/*type vector[size]*/} exp_kind;
...
typedef struct node
{
    ...
    union { uint16_t op;/*tok type*/ int val;/*value assign*/ array array_specs; char* content;/*content of*/} attr; // used in semantic analysis
